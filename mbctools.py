...
def getSingleSeqOrientFileSuffix(loci, samples, rmenu):
        for locus in loci:
                nTotalPlusOrientedSeqCount = 0
                nTotalMinusOrientedSeqCount = 0
                for sample in samples:
                        plusOrientedSeqs = []
                        minusOrientedSeqs = []
                        with open(f"./loci/{locus}/{sample}_singleEnd_orient.tsv", "r") as orientInfo:
                                for line in orientInfo.read().split("\n"):
                                        ...
                                        if len(splitLine) == 4:
                                                if splitLine[1] == '+':
                                                        plusOrientedSeqs.append(splitLine[0])
                                                elif splitLine[1] == '-':
                                                        minusOrientedSeqs.append(splitLine[0])

                        nTotalPlusOrientedSeqCount = nTotalPlusOrientedSeqCount + len(plusOrientedSeqs)
                        nTotalMinusOrientedSeqCount = nTotalMinusOrientedSeqCount + len(minusOrientedSeqs)
                        nTotalSeqCount = len(plusOrientedSeqs) + len(minusOrientedSeqs)
                        if nTotalSeqCount > 0:
                                if len(plusOrientedSeqs) > 0:
                                        # print(warningStyle + f"Locus {locus} / sample {sample}: keeping track of R1/single-end sequences that were initially aligned like the reference (" + str(len(plusOrientedSeqs)) + " out of " + str(nTotalSeqCount) + ")\n" + normalStyle)
                                        with open(f"./loci/{locus}/{sample}_singleEnd_orient_plus.tsv", "w") as orientInfo:
                                                for seq in plusOrientedSeqs:
                                                        orientInfo.write(seq + "\n")
                                if len(minusOrientedSeqs) > 0:
                                        # print(warningStyle + f"Locus {locus} / sample {sample}: keeping track of R1/single-end sequences that were initially aligned differently with the reference (" + str(len(minusOrientedSeqs)) + " out of " + str(nTotalSeqCount) + ")\n" + normalStyle)
                ...
                if strand != "*":
                        ...
                        for sample in samples:
                                ...
                                subprocess.run(["vsearch", "--fastx_getseqs", f"./loci/{locus}/{sample}_singleEnd_orient.fas", "--labels", f"./loci/{locus}/{sample}_singleEnd_orient_" + strand + ".tsv", "--fastaout", f"./loci/{locus}/{sample}_singleEnd_orient_" + strand + ".fas"], stderr=logFile)
                                logFile.close()
                        return "_" + strand
                return ""
...
def trim_2x():
        ...
        if rmenu == "2d":
                ...
                while True:
                        ...
                        orientFileSuffix = getSingleSeqOrientFileSuffix([loc2trim2d], samples, rmenu)
