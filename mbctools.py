...
def promptUser(message, defaultResponse, validResponses, inputType, backFunction, exitMessage):
    ...
    while response is None:
        if firstAttempt:
            ...
            response = input(
                errorStyle + "Wrong input, try again!" + promptStyle + " Accepted entries are " + normalStyle + (
                    "a valid " + (("folder path" if inputType == 4 else (
                        "file path" if inputType == 3 else "number")) + " or ") if inputType > 1 else "") + ", ".join(
                    validResponses) + promptStyle + ": " + normalStyle)
...
def folders():
    ...
    if len(alreadyExisting) > 0:
        sys.stdout.write(
            warningStyle + f"\nThe following folders already exist and will be used in the upcoming analysis: " + normalStyle + ", ".join(
                alreadyExisting))
...
def in_dir_fastq():
    ...
    """
    global dir_fastq
    dir_fastq = promptUser("Enter the FULL PATH of the folder where fastq files are located",
                           f"{current_dir}" + fileSep + "fastq", ["back", "home", "exit"], 4, main_menu1, "")
...
def in_fastq_R1():
    ...
    fastq_R1 = promptUser("Enter the name of the file listing all R1 (or single-end) fastq file names", "fastqR1.txt",
                          ["back", "home", "exit"], 3, main_menu1, "")
...
def in_fastq_R2():
    ...
    fastq_R2 = promptUser(
        "Enter the name of the file listing all R2 fastq file names (file must exist but may be empty if no R2 to process)",
        "fastqR2.txt", ["back", "home", "exit"], 3, main_menu1, "")
...
def in_lociPE():
    ...
    lociPE = promptUser(
        "Enter the name of the file listing names of loci based on paired-end reads (file may be empty if no R2 to process)",
        "lociPE.txt", ["back", "home", "exit"], 3, main_menu1, "")
...
def in_lociSE():
    ...
    lociSE = promptUser("Enter the name of the file containing loci based on R1/single-end reads (or unmerged R1)",
                        "lociSE.txt", ["back", "home", "exit"], 3, main_menu1, "")
...
def in_Samples():
    ...
    Samples = promptUser("Enter the name of the file listing sample names", "samples.txt", ["back", "home", "exit"], 3,
                         main_menu1, "")
...
def in_minsize():
    ...
    global minsize
    minsize = promptUser(
        "Enter the minsize option value for clusters, i.e. the minimum sequence abundance of the retained clusters",
        "8", ["back", "home", "exit"], 2, main_menu1, "")
...
def in_minseqlength():
    ...
    """
    global minseqlength
    minseqlength = promptUser("Enter the minimum length of sequences to keep for any locus", "100",
                              ["back", "home", "exit"], 2, main_menu1, "")
...
def in_alpha():
    ...
    """
    global alpha
    alpha = promptUser("Enter alpha parameter (integer) for the clustering", "2", ["back", "home", "exit"], 2,
                       main_menu1, "")
...
def in_identity():
    ...
    global identity
    identity = promptUser(
        "Enter identity parameter to match the clusters against references, i.e. the identity percentage, enter an integer from 0 to 100",
        "70", ["back", "home", "exit"], 2, main_menu1, "")
...
def in_loc_sel_merged():
    ...
    """
    global loc_sel1
    loc_sel1 = promptUser("Enter the name of the locus analysed by paired-end reads you want to rerun", None,
                          lociPEs + ["back", "home", "exit"], 1, main_menu1, "")
...
def in_loc_sel_r1():
    ...
    loc_sel2 = promptUser("Enter the name of the locus analysed by only single-end (R1) reads you want to rerun", None,
                          lociSEs + ["back", "home", "exit"], 1, main_menu1, "")
...
def in_sam_sel():
    ...
    sam_sel = promptUser("Enter the sample name you want to rerun", None, samples + ["back", "home", "exit"], 1,
                         main_menu1, "")
...
def in_loc2trim_2x():
    ...
    if rmenu == "2a":
        loc2trim2a = promptUser("Enter a LOCUS name based on paired-end mergeable reads you want to analyze", None,
                                lociPEs + ["back", "home", "exit"], 1, main_menu2,
                                f"Selection of minimum sizes according to user-defined thresholds is complete.\nStatistical summary for paired-end based loci --> {current_dir}{fileSep}outputs{fileSep}Stats_option_2a.txt")
        return loc2trim2a

    if rmenu == "2b":
        loc2trim2b = promptUser("Enter a LOCUS name based on single-end R1 reads you want to analyze ", None,
                                lociSEs + ["back", "home", "exit"], 1, main_menu2,
                                f"Selection of minimum sizes according to user-defined thresholds is complete.\nStatistical summary for single-end based loci --> {current_dir}{fileSep}outputs{fileSep}Stats_option_2b.txt")
        return loc2trim2b

    if rmenu == "2c":
        loc2trim2c = promptUser("Enter a LOCUS name based on paired-end mergeable reads you want to analyze", None,
                                lociPEs + ["back", "home", "exit"], 1, main_menu2,
                                f"Selection of minimum sizes according to user-defined thresholds is complete.\nStatistical summary for paired-end based loci --> {current_dir}{fileSep}outputs{fileSep}Stats_option_2c.txt")
        return loc2trim2c

    if rmenu == "2d":
        loc2trim2d = promptUser("Enter a LOCUS name based on single-end R1 reads you want to analyze", None,
                                lociSEs + ["back", "home", "exit"], 1, main_menu2,
...
def in_trim_sample2c(locus):
    ...
    sam2trim2c = promptUser("Enter the name of the sample you want to trim for locus " + locus, None,
                            samples + ["back", "home", "exit"], 1, trim_2x, "")
    return sam2trim2c
...
def in_trim_sample2d(locus):
    ...
    sam2trim2d = promptUser("Enter the name of the sample you want to trim for locus " + locus, None,
                            samples + ["back", "home", "exit"], 1, trim_2x, "")
    return sam2trim2d
...
def in_trim_left(orientFileSuffix):
    ...
    if rmenu == "2a":
        trim_left = promptUser(f"Enter the number of bp of the forward primer for {loc2trim2a} (e.g. 20)", None,
                               ["back", "home", "exit"], 2, main_menu2, "")

    if rmenu == "2b":
        if orientFileSuffix == "_plus":
            trim_left = promptUser(f"Enter the number of bp of the forward primer for {loc2trim2b} (e.g. 20)", None,
                                   ["back", "home", "exit"], 2, main_menu2, "")
        elif orientFileSuffix == "_minus":
            trim_left = promptUser(
                warningStyle + "You chose to keep antisense (-) clusters and therefore should safely be able to use 0 here\n" + promptStyle + f"Enter the number of bp of the forward primer for {loc2trim2b} (e.g. 0)",
                None, ["back", "home", "exit"], 2, main_menu2, "")
        else:
            trim_left = promptUser(
                warningStyle + "Inspite of given advice, you chose to keep both sense (+) and antisense (-) clusters. Now you decide whether or no to trim forward primers ;-)\n" + promptStyle + f"Enter the number of bp of the forward primer for {loc2trim2b}",
                None, ["back", "home", "exit"], 2, main_menu2, "")

    if rmenu == "2c":
        trim_left = promptUser(f"Enter the number of bp of the forward primer for {loc2trim2c} (e.g. 20)", None,
                               ["back", "home", "exit"], 2, main_menu2, "")

    if rmenu == "2d":
        if orientFileSuffix == "_plus":
            trim_left = promptUser(f"Enter the number of bp of the forward primer for {loc2trim2d} (e.g. 20)", None,
                                   ["back", "home", "exit"], 2, main_menu2, "")
        elif orientFileSuffix == "_minus":
            trim_left = promptUser(
                warningStyle + "You chose to keep antisense (-) clusters and therefore should safely be able to use 0 here\n" + promptStyle + f"Enter the number of bp of the forward primer for {loc2trim2d} (e.g. 0)",
                None, ["back", "home", "exit"], 2, main_menu2, "")
        else:
            trim_left = promptUser(
                warningStyle + "Inspite of given advice, you chose to keep both sense (+) and antisense (-) clusters. Now you decide whether or no to trim forward primers ;-)\n" + promptStyle + f"Enter the number of bp of the forward primer for {loc2trim2d}",
                None, ["back", "home", "exit"], 2, main_menu2, "")
    return trim_left
...
def in_trim_right(orientFileSuffix):
    ...
    if rmenu == "2a":
        trim_right = promptUser(f"Enter the number of bp of the reverse primer for {loc2trim2a} (e.g. 22)", None,
                                ["back", "home", "exit"], 2, main_menu2, "")

    if rmenu == "2b":
        if orientFileSuffix == "_minus":
            trim_right = promptUser(f"Enter the number of bp of the reverse primer for {loc2trim2b} (e.g. 20)", None,
                                    ["back", "home", "exit"], 2, main_menu2, "")
        elif orientFileSuffix == "_plus":
            trim_right = promptUser(
                warningStyle + "You chose to keep sense (+) sequences and therefore should safely be able to use 0 here\n" + promptStyle + f"Enter the number of bp of the reverse primer for {loc2trim2b} (e.g. 0)",
                None, ["back", "home", "exit"], 2, main_menu2, "")
        else:
            trim_right = promptUser(
                warningStyle + "Inspite of given advice, you chose to keep both sense (+) and antisense (-) clusters. Now you decide whether or no to trim reverse primers ;-)\n" + promptStyle + f"Enter the number of bp of the reverse primer for {loc2trim2b}",
                None, ["back", "home", "exit"], 2, main_menu2, "")

    if rmenu == "2c":
        trim_right = promptUser(f"Enter the number of bp of the reverse primer for {loc2trim2c} (e.g. 22)", None,
                                ["back", "home", "exit"], 2, main_menu2, "")

    if rmenu == "2d":
        if orientFileSuffix == "_minus":
            trim_right = promptUser(f"Enter the number of bp of the reverse primer for {loc2trim2d} (e.g. 20)", None,
                                    ["back", "home", "exit"], 2, main_menu2, "")
        elif orientFileSuffix == "_plus":
            trim_right = promptUser(
                warningStyle + "You chose to keep sense (+) sequences and therefore should safely be able to use 0 here\n" + promptStyle + f"Enter the number of bp of the reverse primer for {loc2trim2d} (e.g. 0)",
                None, ["back", "home", "exit"], 2, main_menu2, "")
        else:
            trim_right = promptUser(
                warningStyle + "Inspite of given advice, you chose to keep both sense (+) and antisense (-) clusters. Now you decide whether or no to trim reverse primers ;-)\n" + promptStyle + f"Enter the number of bp of the reverse primer for {loc2trim2d}",
                None, ["back", "home", "exit"], 2, main_menu2, "")
    return trim_right
...
def in_ts():
    ...
    if rmenu == "2a":
        ts = promptUser(
            f"Enter the THRESHOLD (integer between 0 and 100) you want to use for locus {loc2trim2a}.\nExample: " + normalStyle + f"if you want to keep only the clusters whose abundance is greater than 5% of the sum of cluster sizes for a given sample with {loc2trim2a}, enter 5" + promptStyle,
            None, ["back", "home", "exit"], 2, main_menu2, "")

    if rmenu == "2b":
        ts = promptUser(
            f"Enter the THRESHOLD (integer between 0 and 100) you want to use for locus {loc2trim2b}.\nExample: " + normalStyle + f"if you want to keep only the clusters whose abundance is greater than 5% of the sum of cluster sizes for a given sample with {loc2trim2b}, enter 5" + promptStyle,
            None, ["back", "home", "exit"], 2, main_menu2, "")

    if rmenu == "2c":
        ts = promptUser(
            f"Enter the THRESHOLD (integer between 0 and 100) you want to use for locus {loc2trim2c}.\nExample: " + normalStyle + f"if you want to keep only the clusters whose abundance is greater than 5% of the sum of cluster sizes for a given sample with {loc2trim2c}, enter 5" + promptStyle,
            None, ["back", "home", "exit"], 2, main_menu2, "")

    if rmenu == "2d":
        ts = promptUser(
            f"Enter the THRESHOLD (integer between 0 and 100) you want to use for locus {loc2trim2d}.\nExample: " + normalStyle + f"if you want to keep only the clusters whose abundance is greater than 5% of the sum of cluster sizes for a given sample with {loc2trim2d}, enter 5" + promptStyle,
            None, ["back", "home", "exit"], 2, main_menu2, "")
...
def quality():
    ...
    with open("scripts/infor1." + scriptExt, "w") as out1:
        i = 0
        out1.write(
            globalErrorOnStopCmd + "\n" + main_stream_message(f'Quality statistical tests on R1 reads for samples:\n'))
        while i < len(samples):
            sample = samples[i]
            fastqr1 = fastqr1s[i]
            i = i + 1
            out1.write(main_stream_message(f' {sample}...') + logFileMessage(
                f'Quality statistical tests on R1 reads for sample {sample}'))
    ...
    if len(lociPEs) > 0:
        with open("scripts/infor2." + scriptExt, "w") as out2:
            ...
            out2.write(globalErrorOnStopCmd + "\n" + main_stream_message(
                f'Quality statistical tests on R2 reads for samples:\n'))
            while i < len(samples):
                sample = samples[i]
                fastqr2 = fastqr2s[i]
                i = i + 1
                out2.write(main_stream_message(f' {sample}...') + logFileMessage(
                    f'Quality statistical tests on R2 reads for sample {sample}'))
...
def merging():
    ...
    with open("scripts/merging." + scriptExt, "w") as out:
        ...
        while i < len(samples):
            ...
            fastqr2 = fastqr2s[i]
            out.write(
                main_stream_message(f" {sample}...") + logFileMessage(f'Merging paired-end reads for sample {sample}') +
...
def fastq2fas():
    ...
    with open("scripts/fqtofas." + scriptExt, "w") as out:
        i = 0
        out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
            f'Converting FASTQ files into FASTA format for samples:\n'))
        while i < len(samples):
            sample = samples[i]
            fastqr1 = fastqr1s[i]
            out.write(main_stream_message(f' {sample}...') + logFileMessage(
                f'Converting FASTQ files into FASTA format for sample {sample}') +
...
--strand both
    ...
    if len(lociSEs) > 0:
        with open("scripts/derep_r1." + scriptExt, "w") as out1:
            ...
            for sample in samples:
                out1.write(main_stream_message(f' {sample}...') + logFileMessage(
                    f'Dereplicating R1/single-end reads for sample {sample}') +
...
--minseqlength int --log ../tmp_files/SN_pairedEnd_cluster.log
...
--minseqlength int --log ../tmp_files/SN_singleEnd_cluster.log
    ...
    if len(lociPEs) > 0 and rmenu in ["1", "1a", "1b"] and len(lociPEs) > 0:
        with open("scripts/cluster." + scriptExt, "w") as out:
            ...
            while i < len(samples):
                sample = samples[i]
                out.write(main_stream_message(f' {sample}...') + logFileMessage(
                    f'Clustering merged reads for sample {sample}') +
                          ...
                          f"--unoise_alph {alpha} --minseqlength {minseqlength} --log "
                          f"../tmp_files/{sample}_pairedEnd_cluster.log" + localErrorOnStopCmd + "\n")
                i = i + 1
            out.write(main_stream_message(f'\n\n'))
    ...
    if len(lociSEs) > 0 and rmenu in ["1", "1a", "1c"]:
        with open("scripts/cluster_r1." + scriptExt, "w") as out:
            ...
            while i < len(samples):
                sample = samples[i]
                out.write(main_stream_message(f' {sample}...') + logFileMessage(
                    f'Clustering R1/single-end reads for sample {sample}') +
                          ...
                          f"--unoise_alpha {alpha} --minseqlength {minseqlength} --log "
                          f"../tmp_files/{sample}_singleEnd_cluster.log" + localErrorOnStopCmd
                          + "\n")
                i = i + 1
            out.write(main_stream_message(f'\n\n'))
    ...
    if rmenu == "1d":
        with open("scripts/cluster_one_sample_1d." + scriptExt, "w") as out:
            ...
            if len(lociPEs) > 0:
                out.write(logFileMessage(
                    f'Clustering merged reads for selected sample {sam_sel}') +
                    ...
                    f"--unoise_alph {alpha} --minseqlength {minseqlength} --log "
                    f"../tmp_files/{sam_sel}_pairedEnd_cluster.log" + localErrorOnStopCmd + "\n")

            if len(lociSEs) > 0:
                out.write(logFileMessage(
                    f'Clustering R1/single-end reads for selected sample {sam_sel}') +
                    ...
                    f"--sizeout --unoise_alph {alpha} --minseqlength {minseqlength} --log "
                    f"../tmp_files/{sam_sel}_singleEnd_cluster.log" + localErrorOnStopCmd + "\n")
            out.write(main_stream_message(f'\n\n'))
...
def chimera_remove():
    ...
    if len(lociPEs) > 0 and rmenu in ["1", "1a", "1b"]:
        with open("scripts/chimera." + scriptExt, "w") as out:
            ...
            for sample in samples:
                out.write(main_stream_message(f' {sample}...') + logFileMessage(
                    f'Detecting and removing chimeras within merged reads of sample {sample}') +
    ...
    if len(lociSEs) > 0 and rmenu in ["1", "1a", "1c"]:
        with open("scripts/chimera_r1." + scriptExt, "w") as out:
            out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
                f'Detecting and removing chimeras within R1/single-end reads of samples:\n'))
            for sample in samples:
                out.write(main_stream_message(f' {sample}...') + logFileMessage(
                    f'Detecting and removing chimeras within R1/single-end reads of sample {sample}') +
    ...
    if rmenu == "1d":
        with open("scripts/chimera_one_sample_1d." + scriptExt, "w") as out:
            ...
            if len(lociPEs) > 0:
                out.write(logFileMessage(
                    f'Detecting and removing chimeras within merged reads of selected sample {sam_sel}') +
            ...
            if len(lociSEs) > 0:
                out.write(logFileMessage(
                    f'Detecting and removing chimeras within R1/single-end reads of selected sample {sam_sel}') +
...
def runloc_merged():
    ...
    with open("scripts/results_by_locusmerged." + scriptExt, "w") as out:
        out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
            f'Affiliating clusters to loci for merged reads of samples:\n'))
        for lociPEb in lociPEs:
            for sample in samples:
                out.write(main_stream_message(
                    f' {sample} vs {lociPEb}...') + logFileMessage(
                    f'Affiliating clusters to locus {lociPEb} for merged reads of sample {sample}') +
...
def runloc_r1():
    ...
    with open("scripts/results_by_locusr1." + scriptExt, "w") as out:
        out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
            f'Affiliating clusters to loci for R1/single-end reads of samples:\n'))
        for locusSEb in lociSEs:
            for sample in samples:
                out.write(main_stream_message(f' {sample} vs {locusSEb}...') + logFileMessage(
                    f'Affiliating clusters to locus {locusSEb} for R1/single-end reads of sample {sample}') +
...
def runlocsel_merged():
    ...
    with open("scripts/results_by_locus_sel." + scriptExt, "w") as out:
        out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
            f'Affiliating clusters to selected locus {loc_sel1} for merged reads of samples:\n'))
        for sample in samples:
            out.write(main_stream_message(f' {sample}...') + logFileMessage(
                f'Affiliating clusters to selected locus {loc_sel1} for merged reads of sample {sample}') +
...
def runlocsel_r1():
    ...
    with open("scripts/results_by_locusr1_sel." + scriptExt, "w") as out:
        out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
            f'Affiliating clusters to selected locus {loc_sel2} for R1/single-end reads of samples:\n'))
        for sample in samples:
            out.write(main_stream_message(f' {sample}...') + logFileMessage(
                f'Affiliating clusters to selected locus {loc_sel2} for R1/single-end reads of sample {sample}') +
                      f"vsearch --usearch_global ../tmp_files/{sample}_singleEnd_cluster_OK.fas --db "
...
def runloc_one_sample_1d():
    ...
    with open("scripts/results_by_locus_merged_1d." + scriptExt, "w") as out:
        out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
            f'Affiliating clusters to loci for merged reads of selected sample {sam_sel}:\n'))
        for lociPEb in lociPEs:
            out.write(main_stream_message(f' {lociPEb}...') + logFileMessage(
                f'Affiliating clusters to locus {lociPEb} for merged reads of selected sample {sam_sel}') +
    ...
    with open("scripts/results_by_locus_R1_1d." + scriptExt, "w") as out1:
        out1.write(globalErrorOnStopCmd + "\n" + main_stream_message(
            f'Affiliating clusters to loci for R1/single-end reads of selected sample {sam_sel}:\n'))
        for locusSEb in lociSEs:
            out1.write(main_stream_message(f' {locusSEb}...') + logFileMessage(
                f'Affiliating clusters to locus {locusSEb} for R1/single-end reads of selected sample {sam_sel}') +
    ...
    if rmenu in ["1", "1a"]:
        if len(lociPEs) > 0:
            with open("scripts/orient_merged_1a." + scriptExt, "w") as out:
                out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
                    f"Orienting all merged reads according to the reference:\n"))
                for locusPEb in lociPEs:
                    for sample in samples:
                        out.write(main_stream_message(f' {sample} vs {locusPEb}...') + logFileMessage(
                            f'Orienting all merged reads according to the reference for locus {locusPEb} and sample {sample}') +
        ...
        if len(lociSEs) > 0:
            with open("scripts/orient_R1_1a." + scriptExt, "w") as out1:
                out1.write(globalErrorOnStopCmd + "\n" + main_stream_message(
                    f"Orienting all R1/single-end reads according to the reference:\n"))
                for locusSEb in lociSEs:
                    for sample in samples:
                        out1.write(main_stream_message(f' {sample} vs {locusSEb}...') + logFileMessage(
                            f'Orienting all R1/single-end reads according to the reference for locus {locusSEb} and sample {sample}') +
        ...
        with open("scripts/orient_merged_1b." + scriptExt, "w") as out:
            out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
                f'Orienting all merged reads according to the reference for selected locus {loc_sel1} and all samples:\n'))
            for sample in samples:
                out.write(main_stream_message(f' {sample}...') + logFileMessage(
                    f'Orienting all merged reads according to the reference for selected locus {loc_sel1} and {sample}') +
        ...
        with open("scripts/orient_R1_1c." + scriptExt, "w") as out:
            out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
                f'Orienting all R1/single-end reads according to the reference for selected locus {loc_sel2} and all samples:\n'))
            for sample in samples:
                out.write(main_stream_message(f' {sample}...') + logFileMessage(
                    f'Orienting all R1/single-end reads according to the reference for selected locus {loc_sel2} and {sample}') +
    ...
    if rmenu == "1d":
        if len(lociPEs) > 0:
            with open("scripts/orient_merged_1d." + scriptExt, "w") as out:
                out.write(globalErrorOnStopCmd + "\n" + main_stream_message(
                    f'Orienting all merged reads of selected sample {sam_sel} for all R1/R2 loci:\n'))
                for locusPEb in lociPEs:
                    out.write(main_stream_message(f' {locusPEb}...') + logFileMessage(
                        f'Orienting all merged reads of selected sample {sam_sel} for locus {locusPEb}') +
        ...
        if len(lociSEs) > 0:
            with open("scripts/orient_R1_1d." + scriptExt, "w") as out18:
                out18.write(main_stream_message(
                    f'Orienting all R1/single-end reads of selected sample {sam_sel} for all R1/single-end loci:\n'))
                for locusSEb in lociSEs:
                    out18.write(main_stream_message(f' {locusSEb}...') + logFileMessage(
                        f'Orienting all R1/single-end reads of selected sample {sam_sel} for locus {locusSEb}') +
...
def getSingleSeqOrientFileSuffix(loci, samples, rmenu):
    for locus in loci:
        ...
        for sample in samples:
            ...
            with open(f"./results_by_locus/{locus}/{sample}_singleEnd_orient.tsv", "r") as orientInfo:
                for line in orientInfo.read().split("\n"):
                    ...
                    if len(splitLine) == 4:
                        if splitLine[1] == '+':
                            nTotalPlusOrientedReadCount = nTotalPlusOrientedReadCount + int(
                                re.sub(r'.*;size=', '', splitLine[0]))
                            plusOrientedClusters.append(splitLine[0])
                        elif splitLine[1] == '-':
                            nTotalMinusOrientedReadCount = nTotalMinusOrientedReadCount + int(
                                re.sub(r'.*;size=', '', splitLine[0]))
        ...
        if nTotalPlusOrientedReadCount + nTotalMinusOrientedReadCount == 0:
            return None
        print(f"\nFor locus {locus}, found " + str(
            nTotalPlusOrientedReadCount) + " sense (+) reads distributed in " + str(
            nTotalPlusOrientedClusterCount) + " clusters and " + str(
            nTotalMinusOrientedReadCount) + " antisense (-) reads distributed in " + str(
            nTotalMinusOrientedClusterCount) + " clusters.\n" + warningStyle
        ...
        if nTotalPlusOrientedReadCount > 0:
            promptOptions = ["+"] + promptOptions
        strand = promptUser(
            "Enter \"+\" to keep only sense clusters, \"-\" to keep only antisense clusters, \"*\" to keep both", None,
            promptOptions, 1, trim_2x, "")
        if strand != "*":
            ...
            for sample in samples:
                if os.path.exists(f"./results_by_locus/{locus}/{sample}_singleEnd_orient_" + strand + ".tsv"):
                    ...
                    subprocess.run(
                        ["vsearch", "--fastx_getseqs", f"./results_by_locus/{locus}/{sample}_singleEnd_orient.fas",
                         "--labels", f"./results_by_locus/{locus}/{sample}_singleEnd_orient_" + strand + ".tsv",
                         "--fastaout", f"./results_by_locus/{locus}/{sample}_singleEnd_orient_" + strand + ".fas"],
                        stderr=logFile)
                    logFile.close()
            return "_" + strand
        return ""
        ...
        with open(f"scripts/runall1.{scriptExt}", "w") as out:
            if not winOS:
                ...
                out.write(start_log_redirect('../outputs/res1.log') +
                          ...
                          + (('"./results_by_locusmerged.' + scriptExt + '", ') if len(lociPEs) > 0 else "")
                          + (('"./results_by_locusr1.' + scriptExt + '", ') if len(lociSEs) > 0 else "")
                          + (('"./orient_merged_1a.' + scriptExt + '"' + (', ' if len(lociSEs) > 0 else "")) if len(
                    lociPEs) > 0 else "")
                          + (('"./orient_R1_1a.' + scriptExt + '"') if len(lociSEs) > 0 else "")
        ...
        with open(f"scripts/runall1d.{scriptExt}", "w") as out:
            if not winOS:
                ...
                out.write(start_log_redirect('../outputs/res1d.log') +
                          ...
                          + (('"./results_by_locus_merged_1d.' + scriptExt + '", ') if len(lociPEs) > 0 else "")
                          + (('"./results_by_locus_R1_1d.' + scriptExt + '", ') if len(lociSEs) > 0 else "")
                          + (('"./orient_merged_1d.' + scriptExt + '"' + (', ' if len(lociSEs) > 0 else "")) if len(
                    lociPEs) > 0 else "")
                          + (('"./orient_R1_1d.' + scriptExt + '"') if len(lociSEs) > 0 else "")
                          + ')\n   For ($i=0; $i -lt $scriptArray.Length; $i++) {\n' +
                          '             $script = $scriptArray[$i]\n' +
...
def trim_2x():
    ...
    if rmenu == "2a":
        ...
        while True:
            ...
            ts = in_ts()
            stat_2a.write(
                f"Locus {loc2trim2a} trimmed {trim_left} bp (forward) and {trim_right} bp (reverse) with threshold set at {ts1}\n")
            for sample in samples:
                ...
                if os.path.exists(f'{sample}_pairedEnd_select.fas') and os.path.getsize(
                        f'{sample}_pairedEnd_select.fas') > 0:
                    ...
                    sys.stdout.write(
                        f"Sum of cluster sizes for {sample} at locus {loc2trim2a} = {a}: with threshold set at {ts[0]}%,{warningStyle} clusters with size >= {b} were retained{successStyle}\n"
                        f"Number of selected clusters for sample {sample}: {nb_selected}\n" + normalStyle)
                else:
                    sys.stdout.write(
                        warningStyle + f"No sequences could be selected for sample {sample} on selected locus\n" + normalStyle)
    ...
    if rmenu == "2b":
        ...
        while True:
            ...
            if orientFileSuffix == None:
                ...
                ts = in_ts()
                stat_2b.write(
                    f"Locus {loc2trim2b} trimmed {trim_left} bp (forward) and {trim_right} bp (reverse) with threshold set at {ts1}\n")
                for sample in samples:
                    if os.path.exists(f"{sample}_singleEnd_orient{orientFileSuffix}.fas") and os.path.getsize(
                            f"{sample}_singleEnd_orient{orientFileSuffix}.fas") > 0:
                        with open(f"{sample}_singleEnd_orient{orientFileSuffix}.fas", 'r') as filin, open(
                                "trim-select." + scriptExt, "w") as out:
                            targets = [line for line in filin if "size" in line]
                            a = 0
                            for target in targets:
                                size = re.search('size=(.+?)$', target).group(1)
                        ...
                        senseReplacements = []
                        if os.path.isfile(sample + "_singleEnd_orient_plus.tsv") and os.path.getsize(
                                sample + "_singleEnd_orient_plus.tsv") > 0:
                        ...
                        if os.path.isfile(sample + "_singleEnd_orient_minus.tsv") and os.path.getsize(
                                sample + "_singleEnd_orient_minus.tsv") > 0:
                        ...
                        nb_selected = open(sample + '_singleEnd_select.fas', 'r').read().count('>') if os.path.exists(
                            sample + '_singleEnd_select.fas') else 0

                        stat_2b.writelines(f"\tNumber of selected clusters for sample {sample}: {nb_selected}\n\n")
                        sys.stdout.write(
                            f"Sum of cluster sizes for {sample} at locus {loc2trim2b} = {a}: with threshold set at {ts[0]}%,{warningStyle} clusters with size >= {b} were retained{successStyle}\n"
                            f"Number of selected clusters for sample {sample}: {nb_selected}\n" + normalStyle)
                        if os.path.exists("tmp"):
                            os.remove("tmp")
                        if os.path.exists("tmp2"):
    ...
    if rmenu == "2c":
        ...
        while True:
            ...
            while True:
                ...
                if not os.path.exists(f"{sam2trim2c}_pairedEnd_orient.fas") or os.path.getsize(
                        f"{sam2trim2c}_pairedEnd_orient.fas") == 0:
                    sys.stdout.write(
                        errorStyle + f"\nNo reads found at locus {loc2trim2c} for sample {sam2trim2c}\n" + normalStyle)
                else:
                    ts = in_ts()
                    with open(f"{sam2trim2c}_pairedEnd_orient.fas", "r") as filin, open("trim-select." + scriptExt,
                                                                                        "w") as filout:
                        targets = [line for line in filin if "size" in line]
                        a = 0
                        for target in targets:
                        ...
                        b = math.ceil(a * float(ts1))
                        stat_2c.write(
                            f"\tSum of cluster sizes for {sam2trim2c} at locus {loc2trim2c} = {a}\n\tWith threshold {ts[0]}, sizes >= {b} for {sam2trim2c} were retained\n")
                    ...
                    sys.stdout.write(
                        f"Sum of cluster sizes for {sam2trim2c} at locus {loc2trim2c} = {a}: with threshold set at {ts[0]}%,{warningStyle} clusters with size >= {b} were retained{successStyle}\n"
        ...
        while True:
            ...
            while True:
                ...
                if orientFileSuffix == None:
                    ...
                    trim_right = in_trim_right(orientFileSuffix)
                    stat_2d.write(
                        f"Locus {loc2trim2d} trimmed at {trim_left} bp (forward) and {trim_right} bp (reverse)\n")
                    ts = in_ts()
                    with open(sam2trim2d + f"_singleEnd_orient{orientFileSuffix}.fas", "r") as filin, open(
                            "trim-select." + scriptExt, "w") as filout:
                        targets = [line for line in filin if "size" in line]
                        a = 0
                        for target in targets:
                            size = re.search('size=(.+?)$', target).group(1)
                        ...
                        b = math.ceil(a * float(ts1))
                        stat_2d.write(
                            f"\tSum of cluster sizes for {sam2trim2d} at locus {loc2trim2d} = {a}\n\tWith threshold {ts[0]}, sizes >= {b} for {sam2trim2d} were retained\n")
                    ...
                    # rename sequence IDs to keep track of their type (merged R1/R2, sense single-end, antisense single-end)
                    senseReplacements = []
                    if os.path.isfile(sam2trim2d + "_singleEnd_orient_plus.tsv") and os.path.getsize(
                            sam2trim2d + "_singleEnd_orient_plus.tsv") > 0:
                        for line in open(sam2trim2d + "_singleEnd_orient_plus.tsv", 'r'):
                            senseReplacements.append((line, line.replace("_R1.", "_R1+.")))
                    antiSenseReplacements = []
                    if os.path.isfile(sam2trim2d + "_singleEnd_orient_minus.tsv") and os.path.getsize(
                            sam2trim2d + "_singleEnd_orient_minus.tsv") > 0:
                    ...
                    sys.stdout.write(
                        f"Sum of cluster sizes for {sam2trim2d} at locus {loc2trim2d} = {a}: with threshold set at {ts[0]}%,{warningStyle} clusters with size >= {b} were retained{successStyle}\n"
...
def concat_3(loci):
    ...
    while loci is None or locusIndex < len(loci):
        loc2cat = promptUser("For which LOCUS do you want to generate a unique sequence file?", None,
                             all_loci + ["back", "home", "exit"], 1, main,
                             f"Results of concatenation session --> {current_dir}{fileSep}outputs{fileSep}Stats_option_3.txt") if loci is None else \
        loci[locusIndex]
        ...
        if len(files2cat) == 0:
            ...
            if loci is not None:
                locusIndex = locusIndex + 1
            elif nb_tot > 0 and "yes" == promptUser(
                    "Do you want to generate dereplicated versions of the above mentioned fasta file? " + normalStyle + "Enter yes or no" + promptStyle,
                    None, ["yes", "no"], 1, None, ""):
                print()
                ...
                derepResults = derepSeveralFastaFiles({loc2cat: loc2cat + '_allseq_select.fasta'},
                                                      loc2cat + '_allseq_select_derep.fasta',
                                                      loc2cat + '_allseq_select_derep.tsv', logFile)
                print(successStyle + str(derepResults[
                                             0]) + " distinct sequences were dereplicated into fasta and tsv files: " + normalStyle + outFasta + ", " + outTsv + "\n")
                logFile.close()
        stat_3.close()

    if loci is not None and anySeqsFound is True and "yes" == promptUser(
            "Do you want to generate dereplicated versions of the above mentioned fasta files? " + normalStyle + "Enter yes or no" + promptStyle,
            None, ["yes", "no"], 1, None, ""):
        print()
        logFile = open(f'{current_dir}{fileSep}outputs{fileSep}res3.log', 'w')
        for locus in loci:
            if locus not in skippedLoci:
                ...
                derepResults = derepSeveralFastaFiles({
                                                          locus: f"{current_dir}{fileSep}results_by_locus{fileSep}{locus}{fileSep}{locus}" + '_allseq_select.fasta'},
                                                      outFasta, outTsv, logFile)
                if derepResults[0] > 0:
                    print(successStyle + str(derepResults[
                                                 0]) + " distinct sequences were dereplicated into fasta and tsv files: " + normalStyle + outFasta + ", " + outTsv + normalStyle + "\n")
                else:
...
def prevent():
    ...
    if os.path.isfile(f"{current_dir}{fileSep}outputs{fileSep}parameters_option_1.cfg") is False:
        ...
        q = promptUser("Do you want to run OPTION 1? " + normalStyle + "Reply yes or exit" + promptStyle, None,
                       ["yes", "exit"], 1, None, "")
        if q == 'yes':
            rmenu = '1'
            menu1()
...
def printHowToCite():
    print(warningStyle + "\nPlease cite this software as follows:" +
          ...
          + "\nChristian Barnabé, Guilhem Sempéré, Vincent Manzanilla, Joel Moo-Millan and Etienne Waleckx.\n" + citationStyle + "https://github.com/GuilhemSempere/mbctools" + normalStyle + "\n\n")
...
def main_menu1():
    ...
    global rmenu
    rmenu = promptUser("Please select an option among those listed above", None,
                       ["1", "1a", "1b", "1c", "1d", "1e", "back", "home", "exit"], 1, main, "")
...
def main_menu2():
    ...
    rmenu = promptUser("Please select an option among those listed above", None,
                       ["2a", "2b", "2c", "2d", "back", "home", "exit"], 1, main, "")
...
def main_menu3():
    ...
    rmenu = promptUser("Please select an option among those listed above", None, ["3a", "3b", "back", "home", "exit"],
                       1, main, "")
...
def main_menu4():
    ...
    rmenu = promptUser("Please select an option among those listed above", None,
                       ["4a", "4b", "4c", "4d", "back", "home", "exit"], 1, main, "")
...
def derepBasedOnIDs(locusToFastaDict, outFastaName, outTsvName):
    ...
    with open(outFastaName, "w") as fastaFile, open(outTsvName, "w") as seqCompositionFile:
            ...
            try:
                ...
                while j < len(lines):
                    ...
                    if line.startswith(">"):
                        ...
                        sampleAndCount = re.sub(r'_(merged|R1).*;', ';', splitIdLine[1].replace("sample=", "")).split(
                            ";size=")
                ...
                print(
                    warningStyle + "File not found: results_by_locus/" + locus + fileSep + locus + '_allseq_select.fasta: skipping locus ' + locus + normalStyle)
        ...
        for seqId in seqSampleAbundances:
            seqName = "seq" + str(i).zfill(padLevel) + "-" + seqOrientations[seqId] + "." + "_".join(
                seqHashToSamplesDict[seqId][0:5]) + ("" if len(seqHashToSamplesDict[seqId]) <= 5 else "...")
...
def derepSeveralFastaFiles(locusToFastaDict, outFastaName, outTsvName, logFile):
    ...
    subprocess.run(
        ["vsearch", "--fastx_filter", outFastaName + ".tmp1", "--relabel_sha1", "--relabel_keep", "--fastaout",
         outFastaName + ".tmp2"], stderr=logFile)
...
def menu4b():
    ...
    blastTextHitTable = promptUser("Enter path to blastn hit-table (text format #7)", None, ["back", "home", "exit"], 3,
                                   main_menu4, "")
...
def menu4c():
    ...
    print(
        f"Any field with '{warningStyle}date{normalStyle}' in its header name will be considered to be the sample {warningStyle}collection date{normalStyle} (recommended format: YYYY-MM-DD)")
    ...
    sampleMetadataFile = promptUser("Enter path to tabulated sample metadata file", None, ["back", "home", "exit"], 3,
                                    main_menu4, "")
            ...
            if splitLine[sampleIndex] not in samples:
                ...
                if collDate != None:
                    try:
                        ...
                        print(errorStyle + "Unable to parse date at line " + str(i) + " \"" + collDate + "\": " + str(
                            e) + normalStyle)
                        menu4c()
...
def menu4d(invokedByUser):
    ...
    zipNow = promptUser(
        "Zip them now to create the final import file? " + normalStyle + "Enter yes or no" + promptStyle, None,
        ["yes", "no"], 1, None, "")
    if zipNow == "yes":
        b = io.BytesIO()
...
def rerun(nextMenu):
    ...
    """
    global next_run
    next_run = promptUser("Do you want to continue with mbctools? " + normalStyle + "Enter yes or no" + promptStyle,
                          None, ["yes", "no"], 1, None, "")
    if next_run == "yes":
        if nextMenu is None:
            main()
...
def main():
    if winOS:
        ...
        with open(permissionTestScript, "w") as out1:
            out1.write("dir\n")
        p = subprocess.run([shellCmd, "." + fileSep + permissionTestScript], stdout=subprocess.DEVNULL,
                           stderr=subprocess.DEVNULL)
        os.remove(permissionTestScript)

        if p.returncode == 1:
            print(
                errorStyle + "Please adjust Execution-Policy to allow script execution" + normalStyle + ", then try again...")
            customExit(1)
